...
| --- | --- | --- | --- | --- | --- |
| `@fi/tool-image-generation` | `image_gen` | `ctx.tools`, `ctx.attachments`, `ctx.fs`, `ctx.credentials` | `tool/call`, `fs/observed for reference images`, `tool/result with durable ImageBlock` | - | The optional FI authorization bundle configures these targets. Provider selection is explicit; no fallback occurs. Schema collection makes no provider request. |
...
Generate one raster image from a prompt, or edit ordered PNG/JPEG/WebP workspace images (up to five for Codex/Grok and three for Antigravity). Choose one configured subscription provider (codex, grok, antigravity); omission uses codex. No provider fallback occurs.
...
Source: [`packages/fi/tool-image-generation/src/tool.ts`](../packages/fi/tool-image-generation/src/tool.ts)

The optional FI authorization bundle configures these targets. Provider selection is explicit; no fallback occurs. Schema collection makes no provider request.

...
| --- | --- | --- | --- | --- | --- |
| `@fi/tool-image-generation` | `image_gen` | `ctx.tools`、`ctx.attachments`、`ctx.fs`、`ctx.credentials` | `tool/call`、`fs/observed 用于参考图片`、`tool/result 携带持久 ImageBlock` | - | 可选的 FI 授权组合包配置这些目标。提供方选择是显式的，不会发生回退。schema 收集不会发出任何提供方请求。 |
...
根据提示词生成一张栅格图片，或编辑有序的 PNG/JPEG/WebP 工作区图片（Codex/Grok 最多五张，Antigravity 最多三张）。选择一个已配置的订阅提供方（codex、grok、antigravity）；省略时使用 codex。不会发生提供方回退。
...
可选的 FI 授权组合包配置这些目标。提供方选择是显式的，不会发生回退。schema 收集不会发出任何提供方请求。

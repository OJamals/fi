...
describe('workspace context instruction discovery', () => {
  ...
  it('defaults dshHome and uses cwd itself as root when no project marker exists', async () => {
    ...
    // the home dirs point at an empty dir so the default ~/.fi holds no global
    // scope. Windows homedir() reads USERPROFILE (not HOME), so both must be
    // stubbed or a real ~/.fi/AGENTS.md would otherwise leak in.
    vi.stubEnv('DSH_HOME', '')
  ...
  it('labels the default DSH home as ~/.fi when HOME points at the configured default', async () => {
    const root = await tempRepo()
    const home = await tempRepo()
    try {
      await write(join(home, '.fi/AGENTS.md'), 'global default rule')
      ...
      expect(files.map(file => file.displayPath)).toEqual(['~/.fi/AGENTS.md'])
    } finally {
      vi.unstubAllEnvs()
      vi.doUnmock('node:os')
      vi.resetModules()
  ...
  it('expands a configured ~/.fi home to the operating-system home directory', async () => {
    const root = await tempRepo()
    const home = await tempRepo()
    try {
      await write(join(home, '.fi/AGENTS.md'), 'global tilde rule')

      vi.resetModules()
      vi.doMock('node:os', () => ({ homedir: () => home }))
      const isolated = await import('@deepseek-ai/dsh-agent-instructions')
      const files = await isolated.discoverBaselineInstructionFiles({ cwd: root, dshHome: '~/.fi' })

      expect(files).toEqual([{ absolutePath: join(home, '.fi/AGENTS.md'), displayPath: '~/.fi/AGENTS.md' }])
    } finally {
      vi.doUnmock('node:os')
      vi.resetModules()
      await rm(root, { recursive: true, force: true })
...
describe('workspace context request injection', () => {
  ...
  it('labels a custom dshHome as DSH_HOME instead of pretending it is ~/.fi', async () => {
    const root = await tempRepo()
    const home = await tempRepo()
    try {

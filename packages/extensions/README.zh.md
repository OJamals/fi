...
extensions 组为 agent 提供只读运行时 API 发现、供程序和浏览器消费者使用的进程内 runner、历史生成插件卡片，以及一个针对本地 Claude Code 与 Codex 插件的、有边界且需要同意的导入器。Creator 模式通过 [Plugin Manager](../boot/plugin-manager/README.zh.md) 安装持久化插件。按需选择检查、Host 执行、Client 执行、浏览器控件或 Claude/Codex 插件兼容性子包。
...
| [`plugin-compat`](plugin-compat/README.zh.md) | 针对本地 Claude Code 与 Codex 插件的有边界扫描、持久化注册表与按 agent 激活 | 提供 `ctx.pluginCompat` |

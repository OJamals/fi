---
description: "面向侧栏和会话首屏的官方 fi 品牌填充，仅在官方构建中生效；供选择或替换品牌呈现的用户与维护者阅读。"
...
本包让以 `official` profile 构建的客户端在侧栏和新会话首屏显示自带 fi 字样的图稿。侧栏把完整构建元数据放在图稿下方，不在旁边重复 fi 文本。其他构建 profile 保留外壳的鱼形标志与本地构建标签。品牌为 fi 的部署应选择本包；使用其他品牌的部署应提供替代品牌包。本包不保留运行时状态，也不影响模型请求。
...
在采用 fi 品牌的部署中，将本插件挂载到浏览器插件名单，然后以 `official` profile 构建客户端，让填充得以注册。

### 选择 profile

`DSH_CLIENT_BUILD_PROFILE` 决定渲染哪个品牌。`official` 构建在侧栏显示自带 fi 字样的图稿，并把构建元数据放在图稿下方；会话首屏显示不带元数据的同一图稿。任何其他取值都让外壳回退——鱼形标志与本地构建标签——保持原样。两种情况下插件都会照常加载并通过校验；只有注册受 profile 门控。

### 替换品牌

自有身份的部署不组合本包，而是组合另一个占据两个侧栏 slot 与会话首屏 slot 的包。占据 slot 是唯一的组合路径；这里不存在任何品牌配置面。

...
it('shares account actions across seats, publishes dialog ownership, and opens contextual support', async ({ start }) => {
  ...
  // fi's Models page declares no DeepSeek sign-in seat; the launcher seat shares the same operations.
  expect(c.ctx.slots.entries('settings.launcher')[0]!.inject!()).toBe(actions)

...
describe('workspace browser rows', () => {
  ...
  it('workspace row menu opens on the ellipsis, renames, and shows the danger delete row', () => {
    ...
    render(<ProjectRowItem
      ...
      actions={{ rename: onRename, delete: onDelete, manageWorktree: onManageWorktree }} t={t}
    />)
    ...
    fireEvent.click(screen.getByRole('button', { name: '工作区“Project”的操作' }))
    fireEvent.click(screen.getByRole('menuitem', { name: '隔离工作树' }))
    expect(onManageWorktree).toHaveBeenCalledOnce()
    expect(screen.queryByRole('menu')).toBeNull()
    fireEvent.click(screen.getByRole('button', { name: '工作区“Project”的操作' }))
  ...
  it('workspace row menu opens at the pointer on right-click and dispatches like the ellipsis', () => {
    ...
    render(<ProjectRowItem
      group={group} onToggle={onToggle} onCreate={vi.fn()}
      actions={{ rename: onRename, delete: onDelete, manageWorktree: vi.fn() }} t={t}
    />)
    const row = screen.getByRole('treeitem')
  ...
  it('session row menu opens without opening the session and Escape closes it', () => {
    ...
    fireEvent.keyDown(document, { key: 'Escape' })
    expect(screen.queryByRole('menu')).toBeNull()
    // Right-click opens the same menu at the pointer without opening the session.

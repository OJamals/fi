 ...
 * Right-click opens the same row menu at the pointer wherever the ... button would.
 */
import { useEffect, useMemo, useRef, useState, type Dispatch, type DragEvent, type MouseEvent, type MutableRefObject, type SetStateAction } from 'react'
...
>) {
  ...
  // Closing through this path also drops the context-click anchor, so a later
  // ... button click does not inherit a stale right-click position.
  const menuOpenState = useMemo((): MenuOpenState => [menuOpen, (open) => {
    if (!open) contextPoint.current = null
    setMenuOpen(open)
  }], [menuOpen])
  ...
  const ownRow = (
    <div
      ...
      draggable={draggable}
      onDragStart={!draggable ? undefined : beginRowDrag(node.id, drag)}

...
启动页只使用原生 DOM 与本地 CSS，因此 bundle 与插件激活失败保持可见：它在一个 spinner 节点上方显示自带 fi 字样的标志，spinner 的 CSS 圆弧随 entry 激活而增长，并逐 entry 报告状态。React 加载前，boot theme 写入的 body 属性会让浅色模式选择默认图稿，并让深色模式选择带透明背景和白色填充的图稿。spinner 及其动画相位会一直保留，直到完整 UI 替换启动页。导入或激活失败的插件会按名称报告并给出原因（缺失服务、导入错误或状态），而不是白屏。

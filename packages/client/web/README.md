...
The boot page uses plain DOM and local CSS, so bundle and plugin-activation failures remain visible: it shows the self-lettered fi mark above one spinner node whose CSS arc grows as entries activate, and reports per-entry status. The boot-theme body attribute selects the default artwork in light mode and the white-fill transparent artwork in dark mode before React loads. The spinner and its animation phase persist until the full UI replaces the boot page. A plugin that fails import or activation is reported by name with the reason (missing service, import failure, or state) instead of a blank page. The console contains the original import error.

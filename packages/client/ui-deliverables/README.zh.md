...
工具栏的第四个控件是"恢复"，对二进制或过大的文件禁用；点击后弹出菜单，列出该文件被捕获的两侧内容——本轮之前、本轮之后。选择一侧会先弹出确认框，说明所选的侧和文件，写入前不做任何改动；确认后调用 Host 受限的恢复操作，并在工具栏下方显示五种结算结果之一：已恢复、因二进制被拒绝、因过大被拒绝、因"分歧"被拒绝（实际文件已不再持有 Host 期望覆盖的内容），或因坐标失效或传输失败而不可用。恢复只写入实际文件；它不追加事件，因此后续轮次自己的对比和 Host 保存的摘要都不受影响；Host 重启后重新打开的对话会像丢失其余对比内容一样丢失恢复控件。
...
- **恢复没有自己的历史或撤销**——确认对话框是唯一的防护；已完成的恢复无法在 review tab 内撤销，且恢复一个已改名文件更早一侧的内容会写入其当前（所列）路径，而不会重建更早的文件名。

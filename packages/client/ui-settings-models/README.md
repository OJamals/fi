...
Open the Models page from the Settings navigation to see configured API-key providers in the main row list. Configured routes with no `apiKeyEnv` whose OAuth flows are offered by a mounted subscription interface appear below its sign-in controls, in the subscription area. An API-key profile stays in the main list even when that provider also offers subscription sign-in. Both locations retain the same edit and delete actions; if no subscription interface is mounted, every configured route stays in the main list. A whole-section provider whose key is not configured anywhere renders as its open setup card instead, but only in the first-run posture and only until the user closes that card. Each card kind owns its own open state, so closing one never discards a draft in another.
...
After the versioned notice step completes, the setup step projects first-run readiness from the same joined snapshot. ANY provider the user can already reach ends it without rendering; only a user with none is routed to the Models page to configure one. Set up later completes only this coordinator pass, and an absent adapter, inactive route, failed join, read-only deployment, or unusable capability completes the step without rendering — Models remains the diagnostic surface.

### Extension slots

The section declares three seats for plugins distributed outside this repository, typed in [`src/client/slot-contract.ts`](src/client/slot-contract.ts) and exported from `./client`. `settings.models.provider-editor` (keyed) replaces the built-in editor for a directory row, dispatched with `entryKey = settingsNs`; its owner props carry the row, its configured state, read-only state, and close callback. The section keeps its built-in editor when no component is registered. `settings.models.provider-card` (keyed) renders inside every card that shows a directory row — a saved row's card in either location, its first-run setup posture, and the add-provider draft — dispatched with `entryKey = settingsNs` and owner props carrying the row's `ConfigurableProviderView`, its configured state, and its confirmed api-key credential state, so one registration under an adapter family's namespace receives every card of that family, hand-declared routes included; the hand-declared draft card has no directory row yet and dispatches nothing until saved. `settings.models.footer` (list) renders after the main rows and the add controls, before grouped subscription rows. A registrant activates through `ctx.slots.inject` with a type-only import of this package's `/client` entry; without registrants the editor keeps its default and the other seats render nothing. A subscription interface registers its currently offered OAuth route ids through `ctx.modelSettingsSubscriptions`, which moves matching keyless configured rows below that footer while leaving API-key profiles in the main list.

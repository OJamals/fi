...
import { refreshIfLoaded } from '../src/client/refresh-if-loaded.ts'
...
describe('ui-settings-models apply', () => {
  it('keeps manual credential onboarding available when the Host disables the automatic step', async () => {
    ...
    try {
      ...
      await plugin.await()
      expect(slots.entries('settings.onboarding').map(entry => entry.options.id)).toEqual(['welcome-notice', 'model-setup'])
...
describe('pushed invalidations', () => {
  ...
  it('routes pushed credential invalidation into the shared onboarding join', async () => {
    ...
    await b.ctx.plugin({ inject: [...inject], apply }).await()
    const entry = b.slots.entries('settings.onboarding')
      .find(candidate => candidate.options.id === 'model-setup')!
    const injected = (
      entry.inject as unknown as
      () => import('../src/client/ModelSetupDialog.tsx').ModelSetupDialogInjected
    )()

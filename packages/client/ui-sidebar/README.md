...
The expanded brand row renders `sidebar.brand.mark` and `sidebar.brand.name` as independent single slots; the collapsed rail renders the same mark slot. The expanded mark owner requests a 48px square and aligns its left edge with the New Session border and macOS traffic lights; its occupant aligns build metadata to the same edge. The collapsed owner requests 24px. The expanded mark receives complete build metadata as `version[-commit][-dirty]`, using `DSH_CLIENT_VERSION`, the optional 7-character `DSH_CLIENT_COMMIT_HASH`, and `DSH_CLIENT_GIT_DIRTY=true`; missing version metadata omits it. Without occupants, the shell uses the fish mark and stacks that metadata beneath a localized local-build label. New Session targets the explicit Workspace used by a scoped action, otherwise the current Session's Workspace, otherwise the most recently active Workspace; when none exists it clears into the blank New Session page.

...
The expanded brand row renders `sidebar.brand.mark` and `sidebar.brand.name` as independent single slots; the collapsed rail renders the same mark slot. The expanded mark owner requests only a 40px square and optically insets the artwork 4px from the New Session border and macOS traffic-light coordinate. Equal 4px padding above and below the artwork plus the trailing 8px margin preserves 12px clearance from both neighboring control groups. The collapsed owner requests 24px. Without occupants, the shell uses the fish mark and stacks complete build metadata beneath a localized local-build label as `version[-commit][-dirty]`, using `DSH_CLIENT_VERSION`, the optional 7-character `DSH_CLIENT_COMMIT_HASH`, and `DSH_CLIENT_GIT_DIRTY=true`; missing version metadata omits the badge. New Session targets the explicit Workspace used by a scoped action, otherwise the current Session's Workspace, otherwise the most recently active Workspace; when none exists it clears into the blank New Session page.

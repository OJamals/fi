...
The client id and secret are never shipped in this repository: GitHub secret scanning flags a Google installed-app client id and secret pair identically to a leaked credential, and the deployment running this adapter owns which Google OAuth client authenticates it. `resolveAntigravityOAuthClient(ctx, refs?, discovery?)` resolves both through `ctx.credentials.resolve(ref)` when the credentials seam is mounted, or the launch environment otherwise — the same fallback `llm-pi-ai` and `fi-web-search-preferences` use. The refs default to `ANTIGRAVITY_OAUTH_CLIENT_ID` and `ANTIGRAVITY_OAUTH_CLIENT_SECRET`; `FiAntigravityConfig`'s `oauthClientIdRef`/`oauthClientSecretRef` fields (both `Volatile<string>`) let a deployment resolve different ref names live, without a restart. Set the two values as environment variables, in a `.env` file (the fi home directory or the launch directory), or as stored credentials (the web Models page writes them) before signing in.

When neither value resolves, sign-in and refresh fall back to local-install auto-discovery before failing: a bounded scan of known Antigravity CLI/app install locations (`FiAntigravityConfig.oauthClientDiscoveryLocations`, defaulting per platform — `agy` on `PATH`, `~/.local/bin/agy`, the macOS `.app` bundle, Linux share/opt/flatpak/snap paths, and the Windows `Programs`/`Program Files` install directories) for the same installed-app client the Antigravity CLI itself carries. A found candidate is trusted only after its SHA-256 matches one of `FiAntigravityConfig.oauthClientDiscoveryFingerprints` (default: the CLI's known client); a verified pair is persisted into fi's credential store under the configured refs through `ctx.credentials`, so later runs skip the scan entirely. Discovery runs at most once per process concurrently, caches a failed scan for `oauthClientDiscoveryNegativeCacheMs`, skips a candidate file over `oauthClientDiscoveryMaxFileBytes` (both `Volatile`), never executes a scanned file, and logs only paths and outcomes, never a matched value. Installing or launching the Antigravity CLI or app is enough to make sign-in work with no manual configuration; missing both values with no local install found still fails the sign-in attempt and any token refresh loud, naming both refs and every place they can be set. `listModels` and the static catalog keep working signed out regardless.

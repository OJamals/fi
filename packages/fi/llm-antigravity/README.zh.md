...
客户端 id 与密钥从不随本仓库分发：GitHub 密钥扫描会把 Google 已安装应用的客户端 id 与密钥对等同于已泄露的凭据来标记，且运行该适配器的部署方拥有决定使用哪个 Google OAuth 客户端进行认证的权限。`resolveAntigravityOAuthClient(ctx, refs?, discovery?)` 在凭据接缝已挂载时通过 `ctx.credentials.resolve(ref)` 解析两者，否则回退到启动环境——这与 `llm-pi-ai` 与 `fi-web-search-preferences` 使用的回退方式相同。这两个引用默认为 `ANTIGRAVITY_OAUTH_CLIENT_ID` 与 `ANTIGRAVITY_OAUTH_CLIENT_SECRET`；`FiAntigravityConfig` 的 `oauthClientIdRef`/`oauthClientSecretRef` 字段（均为 `Volatile<string>`）允许部署方在不重启的情况下实时改用其他引用名。登录前请将这两个值设置为环境变量、写入 `.env` 文件（fi home 目录或启动目录），或作为已存储的凭据（网页版模型页面会写入它们）。

两者均未解析时，登录与刷新会在失败前先回退到本地安装自动发现：对已知 Antigravity CLI/应用安装位置（`FiAntigravityConfig.oauthClientDiscoveryLocations`，按平台给出默认值——`PATH` 上的 `agy`、`~/.local/bin/agy`、macOS 的 `.app` 包、Linux 的 share/opt/flatpak/snap 路径，以及 Windows 的 `Programs`/`Program Files` 安装目录）进行有界扫描，寻找 Antigravity CLI 自身携带的同一个已安装应用客户端。找到的候选值只有在其 SHA-256 与 `FiAntigravityConfig.oauthClientDiscoveryFingerprints`（默认即 CLI 的已知客户端）之一匹配后才会被信任；经验证的一对值会通过 `ctx.credentials` 持久化到 fi 的凭据存储中配置的引用下，因此之后的运行会完全跳过扫描。发现过程在单个进程内同一时刻最多运行一次，失败的扫描会缓存 `oauthClientDiscoveryNegativeCacheMs`，超过 `oauthClientDiscoveryMaxFileBytes`（均为 `Volatile`）的候选文件会被跳过，绝不会执行被扫描的文件，且日志只记录路径与结果，绝不记录匹配到的值。安装或启动 Antigravity CLI 或应用即可让登录在无需手动配置的情况下生效；两者均未配置且未发现本地安装时，登录尝试与任何令牌刷新仍会显式失败，并指出这两个引用名及每个可设置的位置。`listModels` 与静态目录在未登录时仍照常工作。

...
### Live model discovery for subscription routes

A full-catalog route (no `models` list of its own; `modelOverrides` may still reshape individual entries) whose stored credential resolves to `OAuth` — checked with pi-ai's own `Models.getAuth`, never a raw credential read — may see live model ids beyond the installed catalog. The adapter itself ships no provider-specific fetching: it calls the optional `llm-pi-ai/live-models` waterfall, and a plugin that knows how to list one subscription provider's live models (fi's `@fi/provider-compat` for Claude Code, Codex, and Grok) answers with ids and optional display names. A live id absent from the installed catalog is served as a clone of its closest catalog template — the model whose id shares the longest prefix, or the route's first model when none shares any — so it streams through the same api, baseUrl, and compat quirks as its family; only the id and name change. `listModels`, `resolveModel`, and a request naming a live-only id all resolve through this same path, and "Discover models from endpoints" (below) merges the same live ids into a full-catalog route's answer. A curated `models` list, and ambient or API-key authentication, are never augmented — they stay exactly as configured or resolved. Nothing here polls in the background; a listener is expected to cache its own network call and is called again on every `listModels`/discovery/request that reaches this path.
...
Before provider-native authentication, the adapter asks the optional `llm-pi-ai/request-transport` waterfall for header transformation, request-scoped fetch, or WebSocket factory additions. No listener, or an `undefined` result, preserves pi-ai's normal single auth resolution and transport selection. The adapter accepts additions only after pi-ai resolves subscription OAuth, freezes the access token for the lazy SDK call, and verifies final HTTP or WebSocket bearer values before dispatch. WebSocket preparation runs before connection reuse and snapshots its result; provider environment overrides reach the factory. Explicit profile credentials bypass the waterfall, and profile transport choices remain unchanged. The resolver receives nonsecret request facts, not credentials, signals, retries, payloads, or model parameters. Transport callbacks receive authenticated headers and must never log them. Factory owners must dispose their connections and SDK cache partitions without affecting other owners.
...
- **Live model discovery ships no fetcher of its own** — this package only calls the `llm-pi-ai/live-models` waterfall; without a mounted listener that knows one subscription provider's live-listing endpoint, every route keeps exactly its installed catalog. The gate is also narrow by design: only a full-catalog route (no `models` list) whose credential resolves to `OAuth` is ever asked.
...
- **Only a leading in-history `system` message becomes pi-ai's `systemPrompt`** — pi-ai has one system slot, so a later `system` message, or a leading one when `GenerateOptions.system` is also set, folds into a `user` message at its position; provider-specific placement of the prompt follows pi-ai rather than a harness-owned wire override. System images, including a leading system image, fail with `UNSUPPORTED_CONTENT`; assistant images use the assistant-history text fallback instead.

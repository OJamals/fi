...
describe('pi-ai request context conversion', () => {
  ...
  it('converts complete text-only history and drops tool-result images without storage', () => {
    ...
    // The synchronous, no-storage path rejects an image only in `user` history
    // (line-level check in `textOnlyContext`); a tool-result image instead
    // flattens through `flattenText`, which keeps only its text blocks — so a
    // tool result with an image and no text reads as "(no output)".
    expect(toPiContext(request([

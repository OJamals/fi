...
function assertSupportedHistory(messages: readonly RequestMessage[]): void {
  for (const message of messages) {
    ...
    // Assistant images replay as fallback text through toPiAssistant.
...
/** Replace assistant images with fallback text so native-image preparation and offload see only user and tool images. */
function requestImageHistory(
  messages: readonly RequestMessage[],
  resolveImageAccess: ImageAttachmentAccessResolver,
): readonly RequestMessage[] {
  return messages.map((message) => {
    if (message.role !== 'assistant' || !contentHasImage(message.content)) return message
    const content = message.content.map(block => block.type === 'image'
      ? { type: 'text' as const, text: assistantImageHistoryText(block.attachment, resolveImageAccess(block.attachment)) }
      : block)
    return { ...message, content }
  })
}
...
function appendSystemOrAssistant(
  ...
  if (message.role === 'assistant') {
    appendAssistant(message, messages, toolNames, onReplayDegrade, resolveImageAccess)
    return true
  }
  return false
}
...
async function toPiContextWithImages(
  ...
  const requestImages = await prepareRequestImages(safeHistory, attachments, requestImagePolicy, options.signal)
  ...
  for (const [index, message] of exactMessages.entries()) {
    // Assistant replay reads the original message so replay state stays aligned with its non-image blocks.
    const original = split.messages[index] as RequestMessage
    if (appendSystemOrAssistant(original.role === 'assistant' ? original : message, messages, toolNames, onReplayDegrade, resolveImageAccess)) continue
    if (message.role === 'tool') {

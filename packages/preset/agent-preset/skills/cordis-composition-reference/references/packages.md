...
| `@deepseek-ai/dsh-client-ui-brand-official` | no | Official fi brand occupants for the Web client's sidebar and conversation-hero slots |
...
| Package | Config | Description |
|---|---|---|
| `@fi/api-authorization-controller` | no | Remote owner for the authorization seam: the sign-in conversation as a configuration surface drives it |

...
3. **Write English display metadata.** The final `preset.yml` must use English for both `name` and `description`. Replace the copied description with one sentence describing the finished preset, and add a name if you passed none to `copy()`.

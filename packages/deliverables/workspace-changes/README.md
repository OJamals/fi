---
description: "Summarize each top-level turn's changed files from git working-tree snapshots and whole-file captures around file-tool edits, announce them with a workspace/changes Session event, and serve the summary, per-file comparisons, and a guarded restore of either side while the Session lives; configuration, repository requirement, and coverage rules."
...
`ctx.workspaceChanges.restore(sessionId, seq, index, side, signal)` writes the listed file's `'before'` or `'after'` content back to its live path — deleting the path when that side was absent — so a user can undo or redo what one turn did to one file. The write is guarded, not forced: it reads the live file first and compares it against the *opposite* side, and refuses with `diverged` unless the live content still matches exactly, so an edit this summary never observed — a later turn, a manual save, an external process — is never silently discarded. A binary or oversized side refuses the same way `diff` reports it, with no partial write. A successful restore reports `restored` and replaces the live file through a same-directory temporary file and rename, so a concurrent reader never observes a partial write.
...
- A restore does not serialize with an in-flight turn on the same path: the pre-write divergence check makes an unnoticed clobber unlikely, but a concurrent tool edit to the same path can still race the restore write. A restore also appends no event and creates no new turn record; the changed-files card for later turns is unaware a restore happened until their own snapshot runs.

---
description: "用 git 工作树快照和文件工具编辑前后的整文件捕获汇总每个顶层轮次改动的文件，以 workspace/changes Session 事件宣告，并在 Session 存活期间提供摘要、逐文件对比，以及对任一侧内容的受限恢复；配置、仓库要求与覆盖规则。"
...
`ctx.workspaceChanges.restore(sessionId, seq, index, side, signal)` 把所列文件的 `'before'` 或 `'after'` 内容写回其实际路径——该侧为缺失时则删除该路径——从而让用户撤销或重做某一轮对某个文件所做的改动。这次写入是受限的而非强制的：它先读取实际文件，与*相对的另一侧*比较，除非实际内容完全匹配，否则以 `diverged` 拒绝，因此本摘要从未观察到的改动——更晚的一轮、手动保存、外部进程——永远不会被静默丢弃。二进制或超大的一侧按 `diff` 报告的同样方式拒绝，不做部分写入。成功的恢复报告 `restored`，并通过同目录下的临时文件加重命名替换实际文件，因此并发的读取者永远不会看到部分写入。
...
- 恢复不会与同一路径上进行中的轮次串行化：写入前的分歧检查让无声的覆盖不太可能发生，但对同一路径的并发工具编辑仍可能与恢复写入竞争。恢复也不会追加事件或创建新的轮次记录；在后续轮次自己的快照运行之前，其改动文件卡片不会知道发生过恢复。

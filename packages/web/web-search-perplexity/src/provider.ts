...
export class PerplexitySearchProvider implements WebSearchProvider {
  ...
  async search(request: WebSearchRequest, signal?: AbortSignal): Promise<WebSearchResult> {
    ...
    // web-search-deepseek/exa's; only the response error type differs.
    /* jscpd:ignore-start */
    if (!response.ok) {
  ...
  // web-search-deepseek/exa's; only the WebError text names this provider.
  /* jscpd:ignore-start */
...
// These predicates and the abort plumbing are intentionally local: exporting
// generic internals from the public web seam would add more API than these
// pure checks, and the cross-file duplication with web-search-deepseek/exa is
// deliberate (see their own jscpd markers).
/* jscpd:ignore-start */

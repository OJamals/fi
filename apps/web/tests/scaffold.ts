...
export const WELCOME_NOTICE_COPY = {
  zh: {
    title: '内测声明',
    body: 'fi 目前的 0.1 版本仍处在面向开发者进行测试的阶段，还有许多地方需要持续改进和打磨，希望听取广大开发者的反馈建议。预计 fi 的核心插件以及基础 API 都会在接下来的一段时间内快速迭代、持续演化。\n\n我们期待与全球开发者一起，在开源、开放、可复用、可组合的基础设施之上，共同探索智能上限。欢迎全球开发者加入 DSH 插件生态。',
    continueLabel: '继续',
  },
} as const
...
export async function launchWebScaffold(options: LaunchOptions = {}): Promise<WebScaffold> {
  ...
  // real ~/.fi document or credential file.
  ...
  // session is created. Without this a developer's real ~/.fi/skills silently
  ...
  const overlayPatches: PatchOptions[] = [
    ...
    // the owned temp world so ~/.fi, ~/.agents, and a bundled-root env setting

...
describe('web e2e: settings modal and General preferences', () => {
  ...
  it('flips the theme through the Appearance cubes and persists across reload and a distinct port', async () => {
    ...
    expect(light.faviconPaths).toEqual(['/fi-logo.png'])
    ...
    expect(dark.faviconPaths).toEqual(['/fi-logo.png'])
    ...
    expect(reloaded.faviconPaths).toEqual(['/fi-logo.png'])
    ...
    await expect.poll(async () => (await readState()).attr, { timeout: 5_000 }).toBe(true)
    await expect.poll(async () => (await readState()).faviconPaths).toEqual(['/fi-logo-dark-background.png'])
    expectThemeColorSynchronized(await readState())
    ...
    await expect.poll(async () => (await readState()).attr, { timeout: 5_000 }).toBe(false)
    expect((await readState()).faviconPaths).toEqual(['/fi-logo-dark-background.png'])
    await page.emulateMedia({ colorScheme: 'light' })
    await expect.poll(async () => (await readState()).faviconPaths).toEqual(['/fi-logo.png'])
    expectThemeColorSynchronized(await readState())
    await page.keyboard.press('Escape')
    expect(tripwire.pageErrors).toEqual([])
  }, 90_000)

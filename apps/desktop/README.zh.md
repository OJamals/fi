...
| 窗口外观 | 单独的 macOS 标题栏会让原生窗口控件与应用界面割裂。 | 主窗口使用 `hiddenInset`；20px 可拖动行延续侧边栏和主区背景，并承载原生红黄绿按钮。辅助窗口保留标准原生外观。 |
...
生成 GitHub Release 的已签名产物前需设置 `DSH_DESKTOP_AUTO_UPDATE_ENV=production`。打包不要求仓库 token，并继续禁用 electron-builder 自动发布；发布仍是显式 release 操作。已发布的稳定 release 是稳定安装的官方更新流；alpha 安装会跟随已发布的 alpha prerelease。草稿 release 永远不会到达 updater。因为 electron-builder 不会为 GitHub 推断预发布频道，生产打包会根据 Desktop 版本为 GitHub provider 显式设置频道。每个 release 都必须包含平台更新元数据及其引用的全部产物。`fi alpha 1` 仅发布 macOS arm64，因此它的 `alpha-mac.yml` 包含 arm64 ZIP 条目，不包含 x64 条目。

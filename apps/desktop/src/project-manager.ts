...
/**
 * Coalesce a stray fi bundle and upgrade an older built-in prefix without disturbing
 * enabled third-party plugins. A profile created before the fi bundle existed, or one
 * where it was appended out of place, is rewritten with the canonical prefix restored.
 * @param projectDir - Profile directory whose manifest may need the built-in prefix repaired.
...
export class DesktopProjectManager {
  ...
  async applyRelease(): Promise<void> {
    await this.withLock(() => {
      ...
      upgradeProfileBundles(this.paths.profile)

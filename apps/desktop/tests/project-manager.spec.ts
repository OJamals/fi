...
describe('desktop external plugin profile', () => {
  ...
  it('upgrades a profile that predates the fi bundle without changing enabled third-party plugins', async () => {
    const { manager } = setup()
    await manager.applyRelease()
    seedPlugin(manager)
    ...
    expect(plugins(manager)).toEqual([{ name: 'plugin', version: '1.0.0', enabled: true }])
  })

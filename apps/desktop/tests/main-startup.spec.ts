...
const harness = await vi.hoisted(async () => {
  ...
  const app = Object.assign(new EventEmitter(), {
    ...
    getPath: vi.fn((name: string): string => name === 'userData' ? FI_USER_DATA : `desktop-test-${name}`),
  ...
  return {
    ...
    powerMonitor, nativeTheme, trays, FakeTray, backgroundNotice, shellDialog,
...
describe('desktop main startup', () => {
  it('starts packaged FI with an isolated Harness home', async () => {
    harness.app.getPath.mockImplementation((name: string) => name === 'userData' ? FI_USER_DATA : `desktop-test-${name}`)
  ...
  it.each([
    ...
    expect(harness.dialog.showMessageBox).toHaveBeenLastCalledWith(expect.objectContaining({
      type: 'info', title: zh ? '关于 fi' : 'About fi', message: 'fi',
  ...
  it.each(['darwin', 'win32', 'linux'] as const)('limits native titlebar styling to macOS on %s', async (platform) => {
    ...
    if (platform === 'darwin') {
      expect(window.options).toMatchObject({
        titleBarStyle: 'hiddenInset', trafficLightPosition: { x: 14, y: 16 }, vibrancy: 'sidebar', backgroundColor: '#00000000',
      })
  ...
  it('maps Windows caption menus to localized native commands and rejects foreign popup requests', async () => {
    ...
    expect(harness.menu.buildFromTemplate.mock.lastCall![0].map(item => item.label ?? item.type)).toEqual([
      '关于 fi', 'separator', '检查更新…', 'separator', '退出',
    ])
  ...
  it('prepares recovery offscreen and starts one Host before choosing the first visible window', async () => {
    ...
    // The FI Harness home injects DSH_HOME alongside the process environment (see the DSH_HOME specs above);
    // the host environment is never the bare process.env reference.
    expect(harness.hosts[0]!.environment).toMatchObject(process.env)
    expect(harness.hosts[0]!.environment.DSH_HOME).toBeTypeOf('string')

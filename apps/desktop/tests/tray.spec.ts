...
it('shows the application icon with its name as the tooltip and an Open / Quit menu', () => {
  ...
  expect(f.native.setToolTip).toHaveBeenCalledWith('fi')
  expect(labels(native.menus[0]!)).toEqual(['Open fi', 'separator', 'Quit fi'])
...
it('relabels the menu in the current locale and ignores relabel after disposal', () => {
  ...
  f.setLocale('zh')
  f.tray.relabel()
  expect(labels(native.menus[1]!)).toEqual(['打开 fi', 'separator', '退出 fi'])
  f.tray.dispose()
  f.tray.dispose()

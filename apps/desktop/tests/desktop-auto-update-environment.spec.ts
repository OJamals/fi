...
describe('desktop auto-update environment', () => {
  ...
  it('defaults packages and uploads to the test deployment', () => {
    ...
    expect(resolveDesktopAutoUpdateConfig({
      ...
      publish: {
        provider: 'generic',
        url: `https://desktop-updates.example.com/dsh-desk/${RELEASE_ID}/feeds/mac-arm64/`,
      },
    })
  ...
  it.each([undefined, RELEASE_ID, 'unused-test-value'])(
    'pins production packages to the public fi GitHub release feed regardless of test release ID %s',
    (id) => {
      expect(resolveDesktopAutoUpdateConfig({

...
Set `DSH_DESKTOP_AUTO_UPDATE_ENV=production` before producing signed artifacts for a GitHub Release. Packaging does not require a repository token and keeps electron-builder publishing disabled; publication remains an explicit release action. A published stable release is the official stream for stable installations. A preview installation follows published preview prereleases. Draft releases never reach the updater. Production packaging gives the GitHub provider an explicit channel from the Desktop version because electron-builder does not infer prerelease channels for GitHub. Packaged metadata renames the application package to `fi`, making electron-builder's cache directory `fi-updater`. The `afterPack` hook writes that resolved provider, channel, and updater cache directory to the signed application's `Contents/Resources/app-update.yml`, including for the directory stage of the two-lane macOS build where electron-builder does not write it while publication is disabled. Each release carries the platform update metadata and every artifact it references. Current fi Preview releases publish only macOS arm64, so `preview-mac.yml` contains the arm64 ZIP entry and no x64 entry.

...
| State ownership | Sharing the CLI home can make CLI and Desktop contend for the same session write lock; sharing executable dependency graphs also lets either product change the other's packages. | Electron acquires its process-lifetime single-instance lock before profile access and uses its own Harness home for sessions, settings, credentials, storage, and `$DSH_HOME/profiles/desktop`. An explicit `DSH_HOME` opts into a shared home. Executable packages and plugin activation remain Desktop-owned. |
...
| Window chrome | A separate macOS title bar disconnects native controls from the application surfaces. | The main window uses `hiddenInset`; a 32px draggable row continues the sidebar and main backgrounds behind native traffic lights positioned 16px from the top. Auxiliary windows keep standard native chrome. |
...
The [FI data-home decision](../../.agents/notes/implemented/architecture/2026-09-15-fi-desktop-data-home.md) owns the default Harness home and explicit `DSH_HOME` override.

## Installation ownership

By default, Electron uses `<Electron userData>/harness` as its Harness home. On macOS this is `~/Library/Application Support/fi/harness`; other platforms use Electron's `userData` directory. CLI dsh continues to use its own default home. Desktop starts with no CLI session history, settings, credentials, plugins, or storage in its new home; neither home is moved or deleted. Set a nonblank `DSH_HOME` before launching FI to opt into an existing home. Electron and its Host receive the same resolved home, and Electron owns `$DSH_HOME/profiles/desktop`. Its `dependencies` contains only installed external plugins at exact versions; `dsh.profile.bundles` starts with `@deepseek-ai/dsh-base`, `@deepseek-ai/dsh-web-app`, and `@fi/authorization-bundle`, followed by enabled plugins. The signed application supplies dsh, the private Desktop Host, the private FI bundle, and their production packages from `resources/dsh`. Shared package links resolve to those actual directories. Both host and plugins execute in the same bundled upstream Node process, with normal realpath resolution; Desktop does not enable `--preserve-symlinks`. The CLI cannot boot or mutate this profile.

The local startup page exposes fi startup status and available recovery actions; the loaded dsh renderer receives only the desktop protocol marker. The main window carries the same draggable top row while loading and after application navigation. The separate plugin window receives structured list, install, remove, update, and update-check operations; neither renderer receives filesystem access, raw Electron IPC, a shell, or arbitrary pnpm arguments.
...
1. The main window displays a local loading page before profile preparation or backend startup. A fresh profile creates its manifest with the fixed built-in bundle order and shared package links while preserving unrelated files, then starts the actual backend once. An existing profile with the former base-plus-Web prefix gains the FI bundle without changing its third-party suffix. Unchanged startups reuse the profile without scanning installed plugin manifests.
...
Reset deletes every entry in `$DSH_HOME/profiles/desktop` except the held transaction lock, then initializes the built-in profile. It removes Desktop configuration and installed third-party packages without a backup. Sessions, settings, and the Harness-home `.env` are untouched. Shell resource and preload failures use a self-contained document with the available recovery actions and diagnostics; its controls do not require preload.
...
`dev:desktop` builds the current Host, client bundles, Web frontend, and Electron shell, projects the built CLI, private Desktop Host, and private FI bundle closures into a disposable desktop npm project, and launches Electron without downloading the packaged Node.js runtime or resolving dsh from npm:
...
The private FI closure requires the repository-reviewed `@earendil-works/pi-ai` patch. Desktop packs that installed patched dependency into the target package set and installs it from the local tarball. The isolated runtime cannot replace it with the unpatched registry package.
...
`DSH_DESKTOP_AUTO_UPDATE_ENV` selects `test` or `production`; an absent value selects `test`. Test packaging embeds the HTTPS origin from `DOWNLOAD_TEST_ORIGIN`, and the upload commands publish to `DOWNLOAD_TEST_COS_BUCKET` under `_/harness/desktop/stable/<target>/`. Production packaging embeds the source-owned public GitHub provider for [`OJamals/fi`](https://github.com/OJamals/fi); production artifacts belong to a published GitHub Release and the COS upload commands reject that environment.

Package and upload one test target under the same environment:
...
Set `DSH_DESKTOP_AUTO_UPDATE_ENV=production` before producing signed artifacts for a GitHub Release. Packaging does not require a repository token and keeps electron-builder publishing disabled; publication remains an explicit release action. A published stable release is the official stream for stable installations. A preview installation follows published preview prereleases. Draft releases never reach the updater. Production packaging gives the GitHub provider an explicit channel from the Desktop version because electron-builder does not infer prerelease channels for GitHub. Each release carries the platform update metadata and every artifact it references. `fi Preview 04` publishes only macOS arm64, so its `preview-mac.yml` contains the arm64 ZIP entry and no x64 entry.

The test upload validates the completion record, root and Desktop versions, channel metadata, artifact names, sizes, and SHA-512 values before reading its COS credential pair. It uploads immutable versioned artifacts first and version-derived channel metadata last with `no-cache`. Stable builds use `latest-mac.yml` or `latest.yml`; prereleases use their channel name, such as `alpha-mac.yml` or `alpha.yml`.
...
- The desktop shell keeps sessions, settings, credentials, workspaces, storage, and its profile under its own `$DSH_HOME` by default; explicit `DSH_HOME` can share that home with CLI dsh. Executable packages, plugin activation, lockfiles, and package-manager state remain Desktop-owned.

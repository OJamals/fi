...
Set `DSH_DESKTOP_AUTO_UPDATE_ENV=production` before producing signed artifacts for a GitHub Release. Packaging does not require a repository token and keeps electron-builder publishing disabled; publication remains an explicit release action. A published stable release is the official stream for stable installations. An alpha installation follows published alpha prereleases. Draft releases never reach the updater. Production packaging gives the GitHub provider an explicit channel from the Desktop version because electron-builder does not infer prerelease channels for GitHub. Each release carries the platform update metadata and every artifact it references. `fi alpha 1` publishes only macOS arm64, so its `alpha-mac.yml` contains the arm64 ZIP entry and no x64 entry.

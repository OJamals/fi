...
## Phase 6 2026-09-11: one unified section, and the record-updated gap

Live use surfaced three defects, all fixed by one restructuring.

**The Antigravity adopt crash.** The separate Antigravity card's store called `remote.authorization.adopt(key)` and read `adopted.models.join(...)` off the result. Every unary Remote verb answers the `{ok, value}` envelope, not the bare value — the same lesson the earlier `list()` fix pinned, now generalized: unwrap or check `.ok` on every unary call; only the stream verb (`begin`) escapes it. The grant had already committed when the TypeError threw, so the user saw "Sign-in failed" under a succeeded sign-in. The separate card package (`@fi/client-ui-model-signin-antigravity`) is deleted; the unified section drives Antigravity through the same store path as the pi-ai providers, where the envelope has always been unwrapped.

**The missing re-login button.** A grant deletion emits `credentials/record-updated` (grants are records), while the section listened only to `credentials/reference-updated` (API-key references) — so a revoke from any surface left the section rendering the stale stored flag, and no sign-in button ever reappeared. `API_REMOTE_FORWARDED_EVENTS` in `packages/api/remotes/src/remote-events.ts` gains one marked row (`{ event: 'credentials/record-updated', mode: 'emit' }`) — the first fi delta to an upstream runtime file, justified because the allowlist is the only legal way for a host event to reach a browser, and the alternative (polling) is strictly worse. The store's `remove()` also reloads in a `finally`, so a refused or failed revoke never leaves the surface stale either.

...
Users see one fi identity across startup, installed application chrome, and the official browser UI. Native macOS controls belong to the application surface instead of a separate title bar. Brand changes must update the checked-in source artwork and regenerate its desktop and Web derivatives together. Installations and updater metadata use `com.fi.app`; alpha artifacts use version `0.1.6-alpha.1`. Internal tooling and external integrations continue to use their stable DSH identifiers.

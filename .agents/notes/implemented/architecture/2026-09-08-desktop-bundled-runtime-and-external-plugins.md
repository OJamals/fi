...
First launch creates profile metadata — its bundle list starting with the fixed base, Web, and FI authorization-bundle order — without running pnpm, preserving unrelated files. A profile that predates the FI bundle, or carries it outside that fixed prefix, is upgraded in place while preserving its third-party bundle suffix. Each launch computes the runtime resolution from the current installation, including after a compatible release change or application relocation. Node version, platform, or architecture changes preserve installed plugins; pnpm and the loader report installation and compatibility failures.

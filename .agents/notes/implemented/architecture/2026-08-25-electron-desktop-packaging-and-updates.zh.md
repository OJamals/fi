...
`$DSH_HOME/profiles/desktop` 是唯一活动桌面 profile。其可执行包归属及允许解析的目录遵循[内置运行时决策](2026-09-08-desktop-bundled-runtime-and-external-plugins.zh.md)。pnpm 根据正常配置选择 store 和缓存。
...
- `$DSH_HOME/profiles/desktop/node_modules` 保存由共享 Web 插件管理器管理的外部插件。
